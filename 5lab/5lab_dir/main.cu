...
__device__ 
void scan_block(uint32_t *data, uint32_t n){ 
  __shared__ 
  ...
  for (uint32_t i = n>>1; i > 0; i >>= 1, offset<<=1){                    // build sum in place up the tree 
    __syncthreads();    
    if(id < i){ 
  ...
  if(id==0){
    sum = tmp[n-1 + OFFSET(n-1)];
    tmp[n-1 + OFFSET(n-1)] = 0;
  }

  for (uint32_t i = 1; i < n; i <<= 1){ // traverse down tree & build scan 
    offset >>= 1;
    __syncthreads();
    if (id < i){
      uint32_t l = offset*(2*id+1)-1, 
      ...
      temp[l + OFFSET(l)] = temp[r + OFFSET(r)]; 
      temp[r + OFFSET(r)] += t;
    } 
  }  
...
void scan(uint32_t *data, uint32_t size){
  //cerr<<"scan call \\with size = "<<size<<endl;
