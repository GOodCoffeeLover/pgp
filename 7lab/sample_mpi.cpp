...
int main(int argc, char *argv[]) {
  ...
  if (id == 0) {          // Инициализация параметров расчета
    n = 1;            // Размер блока по одному измерению
    nb = 3;           // Размер сетки блоков (процессов) по одному измерению  
    lx = ly = 1.0;
    bc_down = 1.0;
    bc_up = 2.0;
    bc_left = 3.0;
...
/*---------------------------------------------------------------------------------------------------------*/
  ...
  //            buff[i] = data[_i(i, j)];
  //        else
  //          MPI_Recv(buff, n, MPI_DOUBLE, _ib(ib, jb), _ib(ib, jb), MPI_COMM_WORLD, &status);
  //        for(i = 0; i < n; i++)
  //          printf("%.2f ", buff[i]);
  //        if(ib + 1 == nb) {
  //          printf("\n");
  ...
  if (id != 0) {  
    ...
    for(jb = 0; jb < nb; jb++)//nomer stroki processov
      for(j = -1; j <= n; j++)//nomer stroki setki odnogo processa
        for(ib = 0; ib < nb; ib++) {//nomer stolbca processov
          if (_ib(ib, jb) == 0)
            for(i = -1; i <= n; i++)
              buff[i + 1] = data[_i(i, j)];
          else
