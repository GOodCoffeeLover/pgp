...
struct Buffers{
 MPI_Datatype send_left, send_right, 
              recv_left, recv_right,
              
              send_down, send_up, 
              recv_down, recv_up,

              send_front, send_back, 
              recv_front, recv_back;
};
...
void do_math(double* next, double* cur, const Args& arg){
  ...
  #pragma omp parallel private(number_of_threads, tid)
  {
    ...
    for(int k=tid; k<arg.bz; k+=number_of_threads)
      for(int j=0; j<arg.by; j++)
        for(int i=0; i<arg.bx; i++)
          next[_bid(i,j,k)] = 
...
void init_buff(Buffers& buff, const Args& arg){
  ...
  int num_dims=3;
  int grid_size[3] = {arg.bx + 2, arg.by + 2, arg.bz + 2};
  int size[3], pos[3];
  // along YOZ
  size[0]=1;
  size[1]=arg.by; 
  ...
  pos[1]=1;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.send_left);
  MPI_Type_commit(&buff.send_left);

  pos[0]=arg.bx;
  pos[1]=1;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.send_right);
  MPI_Type_commit(&buff.send_right);

  pos[0]=0;
  pos[1]=1;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.recv_left);
  MPI_Type_commit(&buff.recv_left);

  pos[0]=arg.bx+1;
  pos[1]=1;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.recv_right);
  MPI_Type_commit(&buff.recv_right);
  ...
  pos[1]=1;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.send_back);
  MPI_Type_commit(&buff.send_back);

  pos[0]=1;
  pos[1]=arg.by;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.send_front);
  MPI_Type_commit(&buff.send_front);

  pos[0]=1;
  pos[1]=0;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.recv_back);
  MPI_Type_commit(&buff.recv_back);

  pos[0]=1;
  pos[1]=arg.by+1;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.recv_front);
  MPI_Type_commit(&buff.recv_front);
  ...
  pos[1]=1;
  pos[2]=1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.send_down);
  MPI_Type_commit(&buff.send_down);

  pos[0]=1;
  pos[1]=1;
  pos[2]=arg.bz;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.send_up);
  MPI_Type_commit(&buff.send_up);

  pos[0]=1;
  pos[1]=1;
  pos[2]=0;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.recv_down);
  MPI_Type_commit(&buff.recv_down);

  pos[0]=1;
  pos[1]=1;
  pos[2]=arg.bz+1;
  MPI_Type_create_subarray(num_dims, grid_size, size, pos, MPI_ORDER_FORTRAN, MPI_DOUBLE, &buff.recv_up);
  MPI_Type_commit(&buff.recv_up);
}


void sync_edges(double * data, Buffers buff, int id,  const Args& arg){
  ...
  if(z>0)
    MPI_Isend(data, 1, buff.send_down, _pid(x,y,z-1), 0, MPI_COMM_WORLD, req_send+0);
  if(z<arg.z-1)
    MPI_Isend(data, 1, buff.send_up,   _pid(x,y,z+1), 0, MPI_COMM_WORLD, req_send+1);
  
  if(y>0)
    MPI_Isend(data, 1, buff.send_back, _pid(x,y-1,z), 0, MPI_COMM_WORLD, req_send+2);
  if(y<arg.y-1)
    MPI_Isend(data, 1, buff.send_front,_pid(x,y+1,z), 0, MPI_COMM_WORLD, req_send+3);
  
  if(x>0)
    MPI_Isend(data, 1, buff.send_left, _pid(x-1,y,z), 0, MPI_COMM_WORLD, req_send+4);
  if(x<arg.x-1)
    MPI_Isend(data, 1, buff.send_right,_pid(x+1,y,z), 0, MPI_COMM_WORLD, req_send+5);
 ...
 //-----------------------------------------
  ...
  if(z<arg.z-1)
    MPI_Irecv(data, 1, buff.recv_up,   _pid(x,y,z+1), MPI_ANY_TAG, MPI_COMM_WORLD, req_recv+0);
  if(z>0)
    MPI_Irecv(data, 1, buff.recv_down, _pid(x,y,z-1), MPI_ANY_TAG, MPI_COMM_WORLD, req_recv+1);
  
  if(y<arg.y-1)
    MPI_Irecv(data, 1, buff.recv_front,_pid(x,y+1,z), MPI_ANY_TAG, MPI_COMM_WORLD, req_recv+2);
  if(y>0)
    MPI_Irecv(data, 1, buff.recv_back, _pid(x,y-1,z), MPI_ANY_TAG, MPI_COMM_WORLD, req_recv+3);
  
  if(x<arg.x-1)
    MPI_Irecv(data, 1, buff.recv_right,_pid(x+1,y,z), MPI_ANY_TAG, MPI_COMM_WORLD, req_recv+4);
  if(x>0)
    MPI_Irecv(data, 1, buff.recv_left, _pid(x-1,y,z), MPI_ANY_TAG, MPI_COMM_WORLD, req_recv+5);
  


  MPI_Waitall(6, req_send, MPI_STATUS_IGNORE);
  MPI_Waitall(6, req_recv, MPI_STATUS_IGNORE);
}
