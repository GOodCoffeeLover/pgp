...
__constant__ ClassData dev_metrics[classes_numder];
...
void calc_metrics(ClassData metrics[classes_numder], 
                  uchar4* data, 
                  const int& w, 
                  const int& h, 
                  int& NC){
...
__global__ 
void kernel_classify(uchar4* data, const int size, const int NC){
  ...
  for(i=idx; i<size; i += offset ){
    ...
    for(j=0; j<NC; ++j){
      ...
      cur_value_mmp = compute_mmp(p, dev_metrics[j]);
...
int main(){
  ...
  int NC=0;
  calc_metrics(metrics, data, w,h, NC);
  ...
  CSC(cudaMemcpyToSymbol(dev_metrics, metrics, sizeof(ClassData)*classes_numder, 0, cudaMemcpyHostToDevice));

  kernel_classify<<<2048, 1024>>>(dev_data, w*h, NC);
